...
import com.turingoal.qh.ui.adapter.HallRewardAdapter;
...
public class HallRewardFragment extends TgBaseFragment {
    ...
    private HallRewardAdapter adapter = new HallRewardAdapter(); // adapter
    ...
    private void initData(final int getDataType) {
        ...
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_HALL_REWARD, getHttpTaskKey());
        ...
        request.execute(new TgHttpCallback(null) {
            ...
            public void successHandler(TgResponseBean result) { // 加载成功
                if (result.isSuccess()) {
                    ...
                    List<HallRewardBean> data = TgJsonUtil.jsonResultBean2List(result, HallRewardBean.class);
                    if (data != null) {
                        if (TgConstantGetDataType.INIT == getDataType || TgConstantGetDataType.REFRESH == getDataType) {
                            ...
                            // 去重
                            List<HallRewardBean> dataList = new ArrayList<>();
