...
import com.turingoal.qh.ui.adapter.AuditAdapter;
...
@Route(path = ConstantActivityPath.AUDIT)
public class AuditActivity extends TgBaseActivity {
    ...
    private AuditAdapter adapter = new AuditAdapter(); // adapter
    ...
    protected void initialized() {
        tvTitle.setText(R.string.activity_audit);
    ...
    private void initData(final int getDataType) {
        ...
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_AUDIT, getHttpTaskKey());
        ...
        request.execute(new TgHttpCallback(null) {
            ...
            public void successHandler(TgResponseBean result) { // 加载成功
                if (result.isSuccess()) {
                    ...
                    List<AuditBean> data = TgJsonUtil.jsonResultBean2List(result, AuditBean.class);
                    if (data != null) {
                        if (TgConstantGetDataType.INIT == getDataType || TgConstantGetDataType.REFRESH == getDataType) {
                            ...
                            // 去重
                            List<AuditBean> dataList = new ArrayList<>();
