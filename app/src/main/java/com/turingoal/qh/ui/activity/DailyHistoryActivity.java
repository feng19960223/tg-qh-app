...
import com.turingoal.qh.ui.adapter.DailyAdapter;
...
@Route(path = ConstantActivityPath.DAILY_HISTORY)
public class DailyHistoryActivity extends TgBaseActivity {
    ...
    private DailyAdapter adapter = new DailyAdapter(); // adapter
    ...
    private void initData(final int getDataType) {
        ...
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_DAILY_HISTORY, getHttpTaskKey());
        ...
        request.execute(new TgHttpCallback(null) {
            ...
            public void successHandler(TgResponseBean result) { // 加载成功
                if (result.isSuccess()) {
                    ...
                    List<DailyBean> data = TgJsonUtil.jsonResultBean2List(result, DailyBean.class);
                    if (data != null) {
                        if (TgConstantGetDataType.INIT == getDataType || TgConstantGetDataType.REFRESH == getDataType) {
                            ...
                            // 去重
                            List<DailyBean> dataList = new ArrayList<>();
    ...
    public void onViewClicked(final View view) {
        switch (view.getId()) {
            case R.id.ivStart:
                defaultFinish();
                break;
            case R.id.floatingActionButton: // 填写日报
