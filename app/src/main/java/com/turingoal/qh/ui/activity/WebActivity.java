 ...
 * 常用网址页面
 */
@Route(path = ConstantActivityPath.WEB)
public class WebActivity extends TgBaseActivity {
    ...
    private WebAdapter adapter = new WebAdapter(); // adapter
    ...
    private void initData(final int getDataType) {
        ...
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_WEB, getHttpTaskKey());
                ...
                if (result.isSuccess()) {
                    ...
                    if (data != null) {
                        if (TgConstantGetDataType.INIT == getDataType || TgConstantGetDataType.REFRESH == getDataType) {
                            ...
                            // 去重
                            List<WebBean> dataList = new ArrayList<>();
    ...
    public void onViewClicked(final View view) {
        switch (view.getId()) {
            case R.id.ivStart:
                defaultFinish();
                break;
            case R.id.floatingActionButton: // 增加常用网址
                TgSystemHelper.handleIntent(ConstantActivityPath.WEB_ADD);
                break;
            default:
                break;
        }
