...
import com.turingoal.qh.ui.adapter.OvertimeAdapter;
 ...
 * 加班申请历史页面
 */
@Route(path = ConstantActivityPath.OVERTIME)
public class OvertimeActivity extends TgBaseActivity {
    ...
    private OvertimeAdapter adapter = new OvertimeAdapter(); // adapter
    ...
    protected void initialized() {
        tvTitle.setText(R.string.activity_overtime);
    ...
    private void initData(final int getDataType) {
        ...
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_OVERTIME, getHttpTaskKey());
        ...
        request.execute(new TgHttpCallback(null) {
            ...
            public void successHandler(TgResponseBean result) { // 加载成功
                if (result.isSuccess()) {
                    ...
                    List<OvertimeBean> data = TgJsonUtil.jsonResultBean2List(result, OvertimeBean.class);
                    if (data != null) {
                        if (TgConstantGetDataType.INIT == getDataType || TgConstantGetDataType.REFRESH == getDataType) {
                            ...
                            // 去重
                            List<OvertimeBean> dataList = new ArrayList<>();
    ...
    public void onViewClicked(final View view) {
        switch (view.getId()) {
            case R.id.ivStart:
                defaultFinish();
                break;
            case R.id.floatingActionButton: // 填写加班申请
                TgSystemHelper.handleIntent(ConstantActivityPath.OVERTIME_ADD);
                break;
            default:
                break;
        }
