...
import com.turingoal.qh.ui.adapter.HallConventionalAdapter;
...
public class HallConventionalFragment extends TgBaseFragment {
    ...
    private HallConventionalAdapter adapter = new HallConventionalAdapter(); // adapter
    ...
    protected int getLayoutId() {
        return R.layout.fragment_hall_all;
    }
    ...
    private void initData(final int getDataType) {
        ...
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_HALL_CONVENTIONAL, getHttpTaskKey());
        ...
        request.execute(new TgHttpCallback(null) {
            ...
            public void successHandler(TgResponseBean result) { // 加载成功
                if (result.isSuccess()) {
                    ...
                    List<HallConventionalBean> data = TgJsonUtil.jsonResultBean2List(result, HallConventionalBean.class);
                    if (data != null) {
                        if (TgConstantGetDataType.INIT == getDataType || TgConstantGetDataType.REFRESH == getDataType) {
                            ...
                            // 去重
                            List<HallConventionalBean> dataList = new ArrayList<>();
