...
import com.turingoal.qh.ui.adapter.TeamAdapter;
...
@Route(path = ConstantActivityPath.TEAM)
public class TeamActivity extends TgBaseActivity {
    ...
    private TeamAdapter adapter = new TeamAdapter(); // adapter
    ...
    protected void initialized() {
        tvTitle.setText(R.string.activity_team);
    ...
    private void initData(final int getDataType) {
        ...
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_TEAM, getHttpTaskKey());
        ...
        request.execute(new TgHttpCallback(null) {
            ...
            public void successHandler(TgResponseBean result) { // 加载成功
                if (result.isSuccess()) {
                    ...
                    List<TeamBean> data = TgJsonUtil.jsonResultBean2List(result, TeamBean.class);
                    if (data != null) {
                        if (TgConstantGetDataType.INIT == getDataType || TgConstantGetDataType.REFRESH == getDataType) {
                            ...
                            // 去重
                            List<TeamBean> dataList = new ArrayList<>();
