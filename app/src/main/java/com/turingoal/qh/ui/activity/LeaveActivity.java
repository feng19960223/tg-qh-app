...
import com.turingoal.qh.ui.adapter.LeaveAdapter;
 ...
 * 请假历史页面
 */
@Route(path = ConstantActivityPath.LEAVE)
public class LeaveActivity extends TgBaseActivity {
    ...
    private LeaveAdapter adapter = new LeaveAdapter(); // adapter
    ...
    protected void initialized() {
        tvTitle.setText(R.string.activity_leave);
    ...
    private void initData(final int getDataType) {
        ...
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_LEAVE, getHttpTaskKey());
        ...
        request.execute(new TgHttpCallback(null) {
            ...
            public void successHandler(TgResponseBean result) { // 加载成功
                if (result.isSuccess()) {
                    ...
                    List<LeaveBean> data = TgJsonUtil.jsonResultBean2List(result, LeaveBean.class);
                    if (data != null) {
                        if (TgConstantGetDataType.INIT == getDataType || TgConstantGetDataType.REFRESH == getDataType) {
                            ...
                            // 去重
                            List<LeaveBean> dataList = new ArrayList<>();
    ...
    public void onViewClicked(final View view) {
        switch (view.getId()) {
            case R.id.ivStart:
                defaultFinish();
                break;
            case R.id.floatingActionButton: // 填写请假
                TgSystemHelper.handleIntent(ConstantActivityPath.LEAVE_ADD);
                break;
            default:
                break;
        }
