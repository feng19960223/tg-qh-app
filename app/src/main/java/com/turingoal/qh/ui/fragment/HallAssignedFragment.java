...
import com.turingoal.qh.ui.adapter.HallAssignedAdapter;
...
public class HallAssignedFragment extends TgBaseFragment {
    ...
    private HallAssignedAdapter adapter = new HallAssignedAdapter(); // adapter
    ...
    private void initData(final int getDataType) {
        ...
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_HALL_ASSIGNED, getHttpTaskKey());
        ...
        request.execute(new TgHttpCallback(null) {
            ...
            public void successHandler(TgResponseBean result) { // 加载成功
                if (result.isSuccess()) {
                    ...
                    List<HallAssignedBean> data = TgJsonUtil.jsonResultBean2List(result, HallAssignedBean.class);
                    if (data != null) {
                        if (TgConstantGetDataType.INIT == getDataType || TgConstantGetDataType.REFRESH == getDataType) {
                            ...
                            // 去重
                            List<HallAssignedBean> dataList = new ArrayList<>();
