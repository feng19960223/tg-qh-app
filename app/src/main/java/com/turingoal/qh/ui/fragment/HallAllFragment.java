...
import com.turingoal.qh.ui.adapter.HallAllAdapter;
...
public class HallAllFragment extends TgBaseFragment {
    ...
    private HallAllAdapter adapter = new HallAllAdapter(); // adapter
    ...
    private void initData(final int getDataType) {
        ...
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_HALL_ALL, getHttpTaskKey());
        ...
        request.execute(new TgHttpCallback(null) {
            ...
            public void successHandler(TgResponseBean result) { // 加载成功
                if (result.isSuccess()) {
                    ...
                    List<HallAllBean> data = TgJsonUtil.jsonResultBean2List(result, HallAllBean.class);
                    if (data != null) {
                        if (TgConstantGetDataType.INIT == getDataType || TgConstantGetDataType.REFRESH == getDataType) {
                            ...
                            // 去重
                            List<HallAllBean> dataList = new ArrayList<>();
