...
import com.turingoal.qh.ui.adapter.AttributeAdapter;
...
@Route(path = ConstantActivityPath.ATTRIBUTE)
public class AttributeActivity extends TgBaseActivity {
    ...
    private AttributeAdapter adapter = new AttributeAdapter(); // adapter
    ...
    protected void initialized() {
        tvTitle.setText(R.string.activity_attribute);
    ...
    private void initData(final int getDataType) {
        ...
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_ATTRIBUTE, getHttpTaskKey());
        ...
        request.execute(new TgHttpCallback(null) {
            ...
            public void successHandler(TgResponseBean result) { // 加载成功
                if (result.isSuccess()) {
                    ...
                    List<AttributeBean> data = TgJsonUtil.jsonResultBean2List(result, AttributeBean.class);
                    if (data != null) {
                        if (TgConstantGetDataType.INIT == getDataType || TgConstantGetDataType.REFRESH == getDataType) {
                            ...
                            // 去重
                            List<AttributeBean> dataList = new ArrayList<>();
