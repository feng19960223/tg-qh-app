...
import com.turingoal.qh.ui.adapter.MemoAdapter;
...
public class MemoFragment extends TgBaseFragment {
    @BindView(R.id.recyclerView)
    RecyclerView recyclerView; // recyclerView
    @BindView(R.id.swipeRefreshLayout)
    SwipeRefreshLayout swipeRefreshLayout; // 下拉刷新
    private MemoAdapter adapter = new MemoAdapter(); // adapter
    private int limitSize = 40; // 一次加载多少条数据
    private int pageSize = 1; // 第几页
    private static final int SPAN_COUNT = 2; // 瀑布流列数
    ...
    private void initData(final int getDataType) {
        ...
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_MEMO, getHttpTaskKey());
        ...
        request.execute(new TgHttpCallback(null) {
            ...
            public void successHandler(TgResponseBean result) { // 加载成功
                if (result.isSuccess()) {
                    ...
                    List<MemoBean> data = TgJsonUtil.jsonResultBean2List(result, MemoBean.class);
                    if (data != null) {
                        if (TgConstantGetDataType.INIT == getDataType || TgConstantGetDataType.REFRESH == getDataType) {
                            ...
                            // 去重
                            List<MemoBean> dataList = new ArrayList<>();
    ...
    public void onViewClicked() {
        TgSystemHelper.handleIntent(ConstantActivityPath.MEMO_ADD);
    }
}
